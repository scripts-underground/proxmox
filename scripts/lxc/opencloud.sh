...
function install_script() {
  ...
  for server in "${servers[@]}"; do
    until ((attempt >= MAX_ATTEMPTS)); do
      attempt=$((attempt + 1))
      read -r -p "${TAB3}Enter the FQDN of your ${server^} server (ATTEMPT $attempt/$MAX_ATTEMPTS) (eg $server.domain.tld): " fqdn
      if [[ -z "$fqdn" ]]; then
...
EOF
  ...
  fetch_and_deploy_gh_release "OpenCloud" "opencloud-eu/opencloud" "singlefile" "v7.3.0" "/usr/bin" "opencloud-*-linux-$(get_system_arch)"
...
function update_script() {
  ...
  if check_for_gh_release "OpenCloud" "opencloud-eu/opencloud" "${RELEASE}" "each release is tested individually before the version is updated. Please do not open issues for this"; then
    ...
    CLEAN_INSTALL=1 fetch_and_deploy_gh_release "OpenCloud" "opencloud-eu/opencloud" "singlefile" "${RELEASE}" "/usr/bin" "opencloud-*-linux-$(get_system_arch)"
    mv /usr/bin/OpenCloud /usr/bin/opencloud
