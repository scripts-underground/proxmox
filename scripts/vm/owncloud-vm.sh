...
VM_URL="${VM_URL:-https://download.turnkeylinux.org/turnkey/18.3-bookworm-amd64/owncloud_18.3-1_amd64.tar.gz}"
...
function advanced_settings() {
  ...
  if VM_DISK_SIZE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Disk Size in GiB (e.g., 12, 20)" 8 58 "$VM_DISK_SIZE" --title "DISK SIZE" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
...
function post_install_script() {
  msg_ok "Created an ownCloud VM ${CL}${BL}(${VM_HN})"
