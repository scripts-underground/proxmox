...
var_disk="${var_disk:-4}"
VM_URL="${VM_URL:-https://geo.mirror.pkgbuild.com/images/latest/Arch-Linux-x86_64-cloudimg.qcow2}"
...
function header_info {
  ...
  / ___ |/ /__/ /__/ / / /  / /___/ / /_/ / /_/ />  <     | |/ / /  / /
 /_/  |_|\___/\___/_/ /_/  /_____/_/ .___/\__,_/_/|_|     |___/_/  /_/
                                    /_/
EOF
}
...
function default_settings() {
  ...
  VM_MACHINE=""
  VM_DISK_SIZE="${var_disk:-4}G"
  VM_DISK_CACHE=""
  VM_HN="archlinux"
  VM_CPU=""
  VM_CORE_COUNT="${var_cpu:-1}"
  VM_RAM_SIZE="${var_ram:-1024}"
  VM_BRG="vmbr0"
  VM_MAC=""
  VM_VLAN=""
  VM_MTU=""
  ...
  echo -e "${CREATING}${BOLD}${DGN}Creating an Arch Linux VM using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname" 8 58 archlinux --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      VM_HN="archlinux"
      echo -e "${HOSTNAME}${BOLD}${DGN}Hostname: ${BGN}$VM_HN${CL}"
    else
  ...
  if VM_CORE_COUNT=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Allocate CPU Cores" 8 58 1 --title "CORE COUNT" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
  ...
  if VM_RAM_SIZE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Allocate RAM in MiB" 8 58 1024 --title "RAM" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create an Arch Linux VM?" --no-button Do-Over 10 58); then
    echo -e "${CREATING}${BOLD}${DGN}Creating an Arch Linux VM using the above advanced settings${CL}"
  else
    header_info
...
function post_install_script() {
  msg_ok "Created an Arch Linux VM ${CL}${BL}(${VM_HN})"
