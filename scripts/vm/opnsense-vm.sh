...
function default_settings() {
  ...
  VM_HN="opnsense"
  VM_CPU=""
  VM_CORE_COUNT="${var_cpu:-4}"
  VM_RAM_SIZE="${var_ram:-8192}"
  VM_BRG="vmbr0"
  VM_MAC=""
  VM_VLAN=""
  VM_MTU=""
...
function advanced_settings() {
  ...
  if VM_CORE_COUNT=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Allocate CPU Cores" 8 58 4 --title "CORE COUNT" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
  ...
  if VM_RAM_SIZE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Allocate RAM in MiB" 8 58 8192 --title "RAM" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
  ...
  if VM_BRG=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set a LAN Bridge" 8 58 vmbr0 --title "LAN BRIDGE" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_BRG ]; then
      VM_BRG="vmbr0"
    fi
    echo -e "${BRIDGE}${BOLD}${DGN}LAN Bridge: ${BGN}$VM_BRG${CL}"
  else
    exit_script
  fi
...
function post_install_script() {
  msg_ok "Created a OPNsense VM ${CL}${BL}(${VM_HN})"
