...
APP="TurnKey Nextcloud"
var_cpu="${var_cpu:-2}"
var_ram="${var_ram:-2048}"
var_disk="${var_disk:-12}"
VM_URL="${VM_URL:-http://mirror.turnkeylinux.org/turnkeylinux/images/iso/turnkey-nextcloud-18.1-bookworm-amd64.iso}"
...
function default_settings() {
  ...
  echo -e "${CREATING}${BOLD}${DGN}Creating a TurnKey Nextcloud VM using the above default settings${CL}"
}
...
function advanced_settings() {
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create a TurnKey Nextcloud VM?" --no-button Do-Over 10 58); then
    echo -e "${CREATING}${BOLD}${DGN}Creating a TurnKey Nextcloud VM using the above advanced settings${CL}"
  else
    header_info
...
function pre_build_script() {
  ...
  if whiptail --backtitle "Proxmox VE Helper Scripts" --title "TurnKey Nextcloud VM" --yesno "This will create a New TurnKey Nextcloud VM. Proceed?" 10 58; then
    :
  else
...
function post_install_script() {
  msg_ok "Created a TurnKey Nextcloud VM ${CL}${BL}(${VM_HN})"
