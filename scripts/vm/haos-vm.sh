...
function advanced_settings() {
  ...
  if VM_DISK_SIZE=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Disk Size in GiB (e.g., 32, 64)" 8 58 "$VM_DISK_SIZE" --title "DISK SIZE" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
...
function post_install_script() {
  msg_ok "Created a Home Assistant OS VM ${CL}${BL}(${VM_HN})"
