...
- **Pin git commits** — when a script uses `git clone`, optionally set `var_pinned_commit` with a known-good hash. When set, installs pin to that commit instead of the default branch. Script authors bump the pin when they validate a new upstream commit. When not set, the default branch is used (current behavior).
