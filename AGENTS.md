...
- **Expose the app on a standard port** — 80 for HTTP, 443 for HTTPS, and the well-known port for anything with an IANA-assigned service. Non-standard ports are anti-UX: users can't remember them, don't know where to find the UI, and end up guessing. If the upstream app defaults to a non-standard port, find its port config and wire it to the standard.
- **Grep the source for hardcoded ports before writing the script** — Docker apps often hardcode fallback ports in workers, watchers, and schedulers, not just the main service. Set the port env var in EVERY systemd unit that touches the service, including background workers that call the main service over HTTP internally. Missing this in a worker produces errors that don't surface in systemd status.
- **Prefer bare-metal over Docker** — bare-metal installation is the project's goal: simpler, more user-friendly, no Docker dependency. But when an app's Docker stack is inescapable (embedded in its build, config, or runtime), make it transparent to the end user: install Docker as a system dependency, wrap the container lifecycle in systemd units. Exercise judgment on when bare-metal isn't practical.
...
- **Container-consumed vars use `var_lxc_` prefix** — variables that are referenced inside hooks (`install_script`, `update_script`, etc.) must use `var_lxc_*` naming (e.g. `var_lxc_git_repo`, `var_lxc_pinned_commit`). The framework bakes all `var_lxc_*` values from the host shell into guest bundles so they survive the `set -u` boundary. Resource/network vars stay bare `var_*` (host-consumed; never needed inside the container). Hooks may reference `${APP}`, `${NSAPP}`, and `var_lxc_*` directly — no inline defaults required.
- **Pin git commits** — when a script uses `git clone`, optionally set `var_lxc_pinned_commit` with a known-good hash. When set, installs pin to that commit instead of the default branch. Script authors bump the pin when they validate a new upstream commit. When not set, the default branch is used (current behavior).
...
- **Metadata** (`_addon/<slug>.md`): no `cpu`/`ram`/`disk` fields; notes should reference `update-<slug>` / `uninstall-<slug>` command names.
